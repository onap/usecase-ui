...
export class Util {
    ...
    validator(item:object, val:any, index:number,rulesText:any[],validateRulesShow:any[]) {
        ...
        if (val === null || val.replace(/\s*/g, '').length <= 0) {
            ...
            this.getRulesText('Please enter ', item.title, index,rulesText);
            return false
        } else {
            validateRulesShow[index] = false;
        }
        if (isNaN(val) && item.key === 'maxNumberofUEs' && !maxNumberReg.test(val)) {
            validateRulesShow[index] = true;
            this.getRulesText('Only numbers can be entered', '', index,rulesText);
            return false
        } else if (!isNaN(val) && item.key === 'maxNumberofUEs' && !maxNumberReg.test(val)) {
            validateRulesShow[index] = true;
        ...
        if (isNaN(val) && (item.key === 'expDataRateDL' || item.key === 'expDataRateUL') && !expDataRateReg.test(val)) {
            validateRulesShow[index] = true;
            this.getRulesText('Only numbers can be entered', '', index,rulesText);
            return false
        } else if ( !isNaN(val) && (item.key === 'expDataRateDL' || item.key === 'expDataRateUL') && !expDataRateReg.test(val)) {
            validateRulesShow[index] = true;
        ...
        if (isNaN(val) && item.key === 'latency' && !latencyReg.test(val)) {
            validateRulesShow[index] = true;
            this.getRulesText('Only numbers can be entered', '', index,rulesText);
            return false
        } else if (!isNaN(val) && item.key === 'latency' && !latencyReg.test(val)) {
            validateRulesShow[index] = true;
