...
export class Util {
    ...
    getRulesText(words:string, key:string, index:number,rulesText:any[]){
        return rulesText[index] = words + key
    };
    validator(key:string, val:any, index:number,rulesText:any[],validateRulesShow:any[]) {
        ...
        if (val === null || val.replace(/\s*/g, '').length <= 0) {
            ...
            this.getRulesText('Please enter ', key, index,rulesText);
            return false
        } else {
        ...
        if (isNaN(val) && key === 'maxNumberofUEs' && !maxNumberReg.test(val)) {
            ...
            return false
        } else if (!isNaN(val) && key === 'maxNumberofUEs' && !maxNumberReg.test(val)) {
            validateRulesShow[index] = true;
        ...
        if (isNaN(val) && (key === 'expDataRateDL' || key === 'expDataRateUL') && !expDataRateReg.test(val)) {
            ...
            this.getRulesText('Only numbers can be entered', '', index,rulesText);
            return false
        } else if ( !isNaN(val) && (key === 'expDataRateDL' || key === 'expDataRateUL') && !expDataRateReg.test(val)) {
            validateRulesShow[index] = true;
        ...
        if (isNaN(val) && key === 'latency' && !latencyReg.test(val)) {
            ...
            this.getRulesText('Only numbers can be entered', '', index,rulesText);
            return false
        } else if (!isNaN(val) && key === 'latency' && !latencyReg.test(val)) {
            validateRulesShow[index] = true;
