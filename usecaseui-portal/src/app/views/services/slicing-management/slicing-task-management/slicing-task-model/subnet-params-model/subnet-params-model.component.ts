...
export class SubnetParamsModelComponent implements OnInit {
	...
	ngOnChanges() {
		...
		// for (let item of this.coreFormItems) {
		// 	item.value = typeof this.detailData[item.key] === 'undefined'? '': this.detailData[item.key];
		// } 
		// console.log(this.coreFormItems);

		// -------> 2020.08.17  Add 3 parameters for Endpoint, Comment: The following code
        if(this.EndpointEnable){ // end
            this.EndpointInputs = this.title === 'An'
                ?this.formData["an_Endpoint"]
                :this.title === 'Cn'
                    ?this.formData["cn_Endpoint"]
					:[];
			console.log(this.EndpointInputs);
		}
        //-------> Comment: Above code
		if (this.title === 'An') {
		  	this.AreaFormatting();
