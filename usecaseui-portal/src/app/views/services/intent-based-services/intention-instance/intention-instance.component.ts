...
@Component({
  selector: 'app-intention-instance',
  templateUrl: './intention-instance.component.html',
  styleUrls: ['./intention-instance.component.less']
})
export class IntentionInstanceComponent implements OnInit {
  ...
  // init source data
    ...
    this.myHttp.getIntentionInstanceList({
  ...
  verificationIntentionInstance(row): void {
    this.myHttp.verifyIntentionInstance({
      id: row.id
    }).subscribe((response) => {
  ...
  deleteIntentionInstance(row): void {
    this.myHttp.deleteIntentionInstance(row.id).subscribe((response) => {
